...
| 变量 | 路径 |
|------|------|
| `$NOTES_DIR` | `~/Library/CloudStorage/SynologyDrive-documents/obsidian/notes/` |
| `$KNOW_DIR` | `~/Library/CloudStorage/SynologyDrive-documents/obsidian/know/` |
...
| 文件夹路径 | 遍历其中所有 `.pdf` 文件，逐一执行本 skill |

arxiv 论文一律转为 `/html/` 格式，HTML 版本可直接 WebFetch 抓取全文。

提取标题、作者，填入报告头部。

当输入为 PDF 路径且其绝对路径以 `$PAPERS_DIR` 为前缀时，去掉前缀得到相对路径，记入 org 文件头 `#+path:` 字段（替代 `#+source:`）。

#### 去重检查

解析前，在 `$NOTES_DIR` 中递归搜索已有笔记的 `#+source:` 或 `#+path:` 字段，匹配当前论文。若命中，报告已有笔记路径，等用户确认后才重新解析。批量模式下解析完不重复的再总结报告。
