...
| 变量 | 路径 |
|------|------|
| `$NOTES_DIR` | `~/Library/CloudStorage/SynologyDrive-documents/obsidian/research/notes/` |
| `$KNOW_DIR` | `~/Library/CloudStorage/SynologyDrive-documents/obsidian/research/know/` |
| `$PAPERS_DIR` | `~/Library/CloudStorage/SynologyDrive-documents/obsidian/` |

以下所有出现 `$NOTES_DIR` 、 `$KNOW_DIR` 、 `$PAPERS_DIR` 的地方，均使用上表中的实际路径。
...
- 本 skill 中出现的相对路径（如 `./references/template.org`），一律相对于当前 `SKILL.md` 所在目录解析。
- 禁止将 skill 内相对路径解析为执行时工作目录（cwd）下的路径。
- 执行前若需读取 `references/`、`scripts/`、`assets/` 中的文件，先定位 skill base directory，再拼接相对路径。
...
- 输出目录：`$NOTES_DIR/{YYMM}/`（按年月归档，如 `2603` = 2026年3月；不存在则创建）
...
1. `$KNOW_DIR/soul.md` — 世界观、思维范式、核心信念
2. `$KNOW_DIR/memory.md` — 长期记忆、知识连接
...
| 文件夹路径 | 遍历其中所有 `.pdf` 文件（跳过文件名含「翻译」的），每篇论文必须启动独立 Agent 执行本 skill（禁止在同一上下文中解析多篇论文，避免前文分析污染后文判断） |

arxiv 论文一律转为 `/html/` 格式，HTML 版本可直接 WebFetch 抓取全文。

提取标题、作者，填入报告头部。

当输入为 PDF 路径且其绝对路径以 `$PAPERS_DIR` 为前缀时，`#+source:` 记录去掉前缀后的相对路径；否则记录原始路径或 URL。

#### 去重检查

解析前，在 `$NOTES_DIR` 中递归搜索已有笔记的 `#+source:` 字段，匹配当前论文。若命中，报告已有笔记路径，用户有要求再重新解析。批量模式下去重检查在主上下文中完成，筛选出未解析的论文后再逐一启动独立 Agent；已读笔记的内容不得进入解析 Agent 的上下文。解析完所有未重复的论文后在主上下文中总结报告。
...
2. 文件名：`{时间戳}--{简短标题}__read.org`
3. 读取 `./references/template.org` 获取报告结构，按模板填充；该路径相对于当前 `SKILL.md` 所在目录，不是相对于工作目录
4. 写入 `$NOTES_DIR/{YYMM}/{文件名}`（`{YYMM}` 由 `date +%y%m` 获取；目录不存在则创建）
